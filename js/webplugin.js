...
/******* Load CSS *******/
var links = [
  "http://10.58.71.190:8080/webplugin/css/view.css",
];
...
function initSoweb(d) {
  ...
  // 'soweb.directives'
    ...
    //variable - congnt
    $scope.name = "TestCtrl";
    var employeeCode = data.username;//$('#employeeCode').val();
    $scope.pluginButtonShow = "Show";
    $scope.pluginButtonHidden = "Hidden";
    $scope.sidebarButtonShow = "Large";
    $scope.sidebarButtonHidden = "Small";
    $scope.pluginShowButton = $scope.pluginButtonShow;
    $scope.sidebarShowButton = $scope.sidebarButtonHidden;
    $scope.chatThread = [];//[{userId:91,name:"HaiVH2",text:"latestMsg1"},{userId:92,name:"CongNT29",text:"latestMsg2",count:"9+"},{userId:93,name:"TuanNA1",text:"messages3"},{userId:94,name:"ThaoNT24",text:"abc4"},{userId:95,name:"phuongth5",text:"tin nhan 5"},{userId:96,name:"cucntk1",text:"Tin Nhan 6"},{userId:97,name:"hoangpx",text:"messages7"},{userId:98,name:"congnt",text:"latestMsg"},{userId:99,name:"congnt",text:"latestMsg"},{userId:100,name:"congnt",text:"latestMsg"}];
    ...
    socket.on('WebPacket', function(data) {
        ...
        if(data.service == 1){
            ...
            // sendPacket(packet); //off report msg test
    ...
    return {
        link : function(scope, element, attr) {
            scope.$watch(attr.pluginDirective, function(newVal) {
                  if(newVal) {
                      element.addClass('show');
                      scope.pluginShowButton = scope.pluginButtonHidden;
                  } else {
        ...
        link : function(scope, element, attr) {
            scope.$watch(attr.sidebarDirective, function(newVal) {
                  if(newVal) {
                      ...
                      element.removeClass('show');
                      scope.sidebarShowButton = scope.sidebarButtonShow;
                  }                  
            });
        }
    };
...
/******* Load Script *******/
var scripts = [
  // "http://10.58.71.190:8080/webplugin/vendor/socket.io/socket.io.js",
  // "http://10.58.71.190:8080/webplugin/vendor/angular-socket-io/socket.js",
  // "http://10.58.71.190:8080/webplugin/vendor/angular/angular-route.min.js",
  // "http://10.58.71.190:8080/webplugin/vendor/ngstorage/ngStorage.min.js",
  // "http://10.58.71.190:8080/webplugin/vendor/angularjs-scroll-glue/scrollglue.js",
  // "http://10.58.71.190:8080/webplugin/vendor/angular-nanoscroller/jquery.nanoscroller.min.js",
  // "http://10.58.71.190:8080/webplugin/vendor/angular-nanoscroller/scrollable.js",
  // "http://10.58.71.190:8080/webplugin/js/config.js",
  // "http://10.58.71.190:8080/webplugin/js/app.js",
  "http://10.58.71.190:8080/webplugin/js/controllers.js",
  "http://10.58.71.190:8080/webplugin/js/directives.js"
  ];
...
$(document).ready(function(){
    ...
    /******* Load HTML *******/
    $('body').append('<div id="soweb" ng-app="soweb" ng-controller="HomeCtrl" plugin-directive="!statePlugin"></div>');
    $('#soweb').append('<div id="button-toggle" ng-click="togglePlugin()"><span>{{pluginShowButton}}</span></div>');
    $('#soweb').append('<div id="container" sidebar-directive="!stateSidebar"></div>');
    // 
    $('#container').append('<div id="sidebar"></div><div id="main"></div>');
    // 
    $('#sidebar').append('<div class="top sidebar_top"></div><div class="sidebar_content"></div>');
    $('#sidebar .top.sidebar_top').html('<strong>{{myFullname}}</strong> <br> <small title="{{myOrganization}}">{{myOrganization}}</small>');
    $('#sidebar .sidebar_content').append('<div class="tabs_title"></div><div class="tabs_search"></div><div class="tabs_content"></div>');
    $('#sidebar .sidebar_content .tabs_title').append('<div ng-class="{ active: isSet(1) }" ng-click="setTab(1)"><span>IM</span></div>');
    $('#sidebar .sidebar_content .tabs_title').append('<div ng-class="{ active: isSet(2) }" ng-click="setTab(2)"><span>C</span></div>');
    $('#sidebar .sidebar_content .tabs_title').append('<div ng-class="{ active: isSet(3) }" ng-click="setTab(3)"><span>S</span></div>');
    $('#sidebar .sidebar_content .tabs_search').append('<input ng-model="sidebarSearch" type="search" placeholder="Search">');
    $('#sidebar .sidebar_content .tabs_content').append('<div id="tab-pane-1" class="tab-pane" ng-show="isSet(1)" scrollable static ></div>');
    $('#sidebar .sidebar_content .tabs_content').append('<div id="tab-pane-2" class="tab-pane" ng-show="isSet(2)" scrollable static ></div>');
    $('#sidebar .sidebar_content .tabs_content').append('<div id="tab-pane-3" class="tab-pane" ng-show="isSet(3)" scrollable static >Setting</div>');
    $('#sidebar #tab-pane-1').append('<div class="im_dialog" ng-repeat="c in chatThread|filter:sidebarSearch" ng-click="threadChatClick(c.userId,c.name)" ng-class="{active: c.userId==selected}"></div>');
    $('#sidebar #tab-pane-1 .im_dialog').append('<div class="im_dialog_photo"><div class="im_dialog_photo" ng-class="randomAvatarColor(c.userId)">{{getKeyFromFullname(c.name)}}</div></div><div class="im_dialog_message"></div>');
    $('#sidebar #tab-pane-1 .im_dialog .im_dialog_message').append('<span class="user" id=""><strong title="{{c.name}}">{{c.name}}</strong><span class="badge {{c.count}}" ng-show="c.count != null">{{convertUncountMessageToText(c.count)}}</span></span>');
    $('#sidebar #tab-pane-1 .im_dialog .im_dialog_message').append('<span class="message" id=""><small title="{{c.text}}">{{c.text}}</small><span class="latestVisitedDate" id=""><small>{{c.lastVisitDate}}</small></span></span>');
    $('#sidebar #tab-pane-2').append('<div class="contact im_dialog" ng-repeat="fr in listOrgFriend | filter:sidebarSearch" ng-click="threadChatClick(fr.userId,fr.fullname)" ng-class="{active: fr.userId == selected}" ng-hide="fr.userId < 0 || fr.userId == myUserId"></div>');
    $('#sidebar #tab-pane-2 .im_dialog').append('<div class="im_dialog_photo"><div class="im_dialog_photo" ng-class="randomAvatarColor(fr.userId)">{{getKeyFromFullname(fr.fullname)}}</div></div><div class="im_dialog_message"></div>');
    $('#sidebar #tab-pane-2 .im_dialog .im_dialog_message').append('<span class="user" id=""><strong title="{{fr.fullname}}">{{fr.fullname}}</strong></span>');
    $('#sidebar #tab-pane-2 .im_dialog .im_dialog_message').append('<span class="message" id=""><span class="latestVisitedDate" id=""><small>{{fr.lastVisitDate}}</small></span></span>');
    // 
    $('#main').append('<div class="top"></div><div class="main_content"></div>');
    $('#main .top').append('<button ng-click="toggleSidebar()">{{sidebarShowButton}}</button><strong>{{currentChatName}}</strong><small>{{currentStautus}}</small>');
    $('#main .main_content').append('<div class="im_history" scrollable static scroll-glue></div><div class="im_bottom"></div>');
    $('#main .main_content .im_history').append('<div class="message" ng-repeat="m in messages"></div>');
    $('#main .main_content .im_history .message').append('<div class="avatar"><span class="msg-avatar" ng-class="randomAvatarColor(m.userId)">{{getKeyFromFullname(m.name)||"UN"}}</span></div>');
    $('#main .main_content .im_history .message').append('<div class="message"><p><strong>{{m.name||"Unknown"}}</strong></p><p>{{m.text}}</p></div>');
    $('#main .main_content .im_bottom').append('<div class="avatar"></div><form name="msgForm"></form>');
    $('#main .main_content .im_bottom form').append('<div class="input"><input type="text" ng-trim="true" id="msg" ng-keypress="keypress($event)" ng-model="msg" placeholder="Write a message..."></div>');
    $('#main .main_content .im_bottom form').append('<div class=""><button type="button" ng-click="sendMessage()" class="btn btn-sm" ng-disabled="!msg.length">Send</button></div>');

    angular.bootstrap(document.getElementById("soweb"),['soweb']);
